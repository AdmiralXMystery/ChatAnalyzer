...
from core.chat_report_generator_group import GroupChatReportGenerator
...
FEATURES = [
    ("daily_activity",   "Средняя суточная активность", True),
    ("activity_chart",   "График активности (автомасштаб)", True),
    ("top_senders",      "Топ-15 активных участников", True),
    ("longest_messages", "Топ-10 длинных сообщений", True),
    ("longest_pause",    "Самый долгий перерыв + контекст", True),
    ("top_words",        "Топ слов и карта слов", True),
    ("emoji",            "Статистика эмодзи (общая)", True),
    ("profanity",        "Нецензурная лексика (18+)", True),
    ("wordcloud",        "Облако слов (Wordcloud)", True),
]


class GroupAnalyzerPage(QWidget):
    def __init__(self, main_window=None):
        ...
        root.addWidget(PageHeader(
            "Анализ групповой переписки",
            "HTML-отчёт по групповой переписке (3+ участников)"
        ))
        ...
        for key, label, default in FEATURES:
            ...
            tg.setMinimumHeight(30)  # как в page_analyzer.py
            self.feature_toggles[key] = tg
            feat_card.add(tg)  # ← напрямую, без обёртки
        ...
        # ---------- ПРАВАЯ КОЛОНКА: выбор файла + журнал ----------
    ...
    async def _run(self, csv_path_str: str):
        try:
            ...
            chat_name = csv_path.stem.replace("messages_", "")
            cache_dir = Path(".").resolve() / "cache" / f"group_{chat_name}"
            cache_dir.mkdir(parents=True, exist_ok=True)

            self.log.log(f"🚀 Изолированная папка кэша группового чата:")
    ...
    def _run_pipeline(self, csv_path: Path, cache_dir: Path, enabled_features: dict):
        ...
        try:
            generator = GroupChatReportGenerator(
